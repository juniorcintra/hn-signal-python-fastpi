"""
Hacker News scraper using BeautifulSoup (static content).
"""
...
class HNScraper(BaseScraper):
    """BeautifulSoup-based scraper for Hacker News (static HTML)."""
    ...
    async def fetch(self, url: str) -> str:
        """Fetch HTML from URL using httpx (async)."""
    ...
    def parse(self, html: str) -> list[dict]:
        ...
        for row in story_rows:
            try:
                ...
                rank = self._parse_int(rank_el.get_text() if rank_el else "0")
                ...
                if subtext_row:
                    ...
                    points = self._parse_int(score_el.get_text() if score_el else None)
                    author = author_el.get_text(strip=True) if author_el else "unknown"

                    for link in subtext_row.select("a"):
                        link_text = link.get_text(strip=True).lower()
                        if "comment" in link_text:
                            comments_count = self._parse_int(link_text)
                            break
